...
| **iS-NexFlow** | 项目执行操作系统 | B·业务线级 | 迭代推演中 | 🟡 已立项 | iS-SCO | 将项目执行过程结构化与可计算，作为引擎业务输出 |
...
### 4.1 待决策事项
* **GPT 侧全量同步**：本次（2026-04-09）完成了大规模文件更新，GPT 侧尚未同步任何文件，需要用户完成一次全量上传（见第2节清单）。
...
* **iS-Synexa Step 0B**：超智建设项目需推进至 Step 0B（启动检查），确认品牌叙事方向、官网目标受众、视觉规范起点。
