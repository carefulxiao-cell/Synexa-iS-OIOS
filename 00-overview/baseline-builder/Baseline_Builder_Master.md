...
stat_rule: [统计函数] | [章节关键词] | [中文标签] | [英文标签]
stat_rule: ...（可多行，每行一个卡片）
-->
```
...
| `stat_rule` | ✅ | 统计规则，每行一个 statgrid 卡片，见 16.4 |

### 16.4 stat_rule 语法规范

**格式**：`stat_rule: [函数名] | [章节关键词] | [中文标签] | [英文标签]`
...
- statgrid 数字**自动统计，不手写固定值**；每次 md 内容更新后重跑脚本，数字自动更新
- COVER 块不影响 md 正文阅读，AI 读取 md 时自动忽略注释块
- 每个基线文件的 `stat_rule` 按自身内容定义，体现个性化高价值信息
- `topline` 格式统一：`SYNEXA · [项目代号] · INTERNAL SSOT · [层级]`
- COVER 块放在 md 文件**第一行**，在所有正文内容之前

---

*BBM V1.3 · 2026-06 · SYNEXA INTERNAL SSOT*
