...
title: 超智科技·数智协同基线
en: Synexa Digital-Intelligence Collaboration Baseline V0.4
...
| Short Name | DICB |
| Version | V0.4 |
| Upper Source | Synexa Company Intro V3.11 |
| Maintainer | iS-Core |
...
### CH05.5 项目生命周期刻度（Project Lifecycle Scale）

为了防止项目陷入“想到哪做到哪”的混乱，DIC 为所有项目规定了统一的生命周期刻度。每个项目必须在 Context OS 和 PCS 中明确标注当前所处的 Step。

| 刻度 | 阶段名称 | 核心目标 | 完成标准与输出物 | 状态标签 |
|---|---|---|---|---|
| **Step 0A** | 最小定义 | 在 iS-Core 完成项目“准生证”注册 | 确定项目代号、一句话定义、层级归位、优先级 | 🟡 已立项 |
| **Step 0B** | 启动检查 | 开工前的对齐与资源确认 | 目标对齐、资源确认、PCS 建立、作战室就绪 | 🟡 待启动 |
| **Step 1** | 战略推演 | 明确业务逻辑与路径设计 | 业务逻辑、用户画像、竞争判断、路径设计 | 🟢 推进中 |
| **Step 2** | 结构设计 | 搭建项目骨架与接口 | 架构图、模块拆解、接口定义、依赖梳理 | 🟢 推进中 |
| **Step 3** | 业务建模 | 深度执行与数据结构化 | 数据字典、功能模块、场景配置、流程图 | 🟢 推进中 |
| **Step 4** | 原型验证 | 最小闭环的真实测试 | 最小可行版本（MVP）、真实场景测试反馈 | 🟢 推进中 |
| **Step 5** | 交付落地 | 生产部署与经验回流 | 生产部署、运营移交、经验沉淀、归档 | ✅ 已交付 |
...
| V0.4 | Registry Version | Tool / Agent / Skill Registry 建立 |
...
{
  "document_id": "Synexa_DIC_V0.4",
  "zh_name": "超智科技·数智协同基线",
  "en_name": "Synexa Digital-Intelligence Collaboration Baseline",
  "short_name": "DICB",
  "version": "V0.4",
  "status": "Registry Version",
  "upper_source": "Synexa_Company_Intro_V3.11",
  "maintainer": "iS-Core",
...
| **L3 支撑层** | S² 层（SOP + Skill 协同神经体层） | S1-S3 | iS-Proxima（近场感知）/ iS-Vena（协同数据脉）/ iS-Cortex（协同皮层） |
...
| V0.2 | 2026-07-02 | 对齐 G/A/E/N/P 五层架构，更新 Appendix I 映射表 | 全局工作空间治理 | iS-Core |
| V0.4 | 2026-07-03 | N层修正为S²层，新增 CH05.5 项目生命周期刻度 | 全局架构与项目管理 | iS-Core |
