...
| **Nex2U** | 超级健康管理 | 健康餐食全局管理引擎 | 医疗机构、养老院、健康管理机构（B端决策层） | 规则 SSOT，面向 B 端决策层与技术对接方 |
| **NexChef** | 超级餐食服务 | 消费与应用交互引擎 | C端消费者、B端客户操作层（点餐/出品） | Nex2U 下游出口，含点餐系统，不自建规则 |
...
| `DOMAIN_REGISTRY_v1.0.md` | 2026-04-26 | ⏳ 待上传 | 域名与基础设施注册表 v1.0（新建，含品牌域/系统域分工逻辑与完整分配表） |
| `CONTEXT_OS.md` | 2026-04-26 | ⏳ 待上传 | 本文件（**v2.6**，引擎四大→七大，Nexsply 定位升级，Nex2U/NexChef 架构裁决） |
